 ...
 , mCornersFound(false)
{
    if (image.channels() == 1)
    {
        cv::cvtColor(image, mSketch, COLOR_GRAY2BGR);
        image.copyTo(mImage);
    }
    else
    {
        image.copyTo(mSketch);
        cv::cvtColor(image, mImage, COLOR_BGR2GRAY);
    }
}
                                          ...
                                          int flags)
{
    ...
    // MARTIN: Set to "false"
    if (image.channels() != 1 || (flags & CALIB_CB_NORMALIZE_IMAGE))
    {
    ...
    for (int k = 0; k < 6; ++k)
    {
        for (int dilations = minDilations; dilations <= maxDilations; ++dilations)
        {
            ...
            if (flags & CALIB_CB_ADAPTIVE_THRESH)
            {
                ...
                // convert to binary
                cv::adaptiveThreshold(img, thresh_img, 255, ADAPTIVE_THRESH_MEAN_C, THRESH_BINARY, blockSize, (k/2)*5);
            }
            else
            {
                ...
                cv::threshold(img, thresh_img, thresh_level, 255, THRESH_BINARY);
            }
            ...
            cv::Mat kernel1 = cv::getStructuringElement(MORPH_CROSS, cv::Size(3,3), cv::Point(1,1));
            cv::Mat kernel2 = cv::getStructuringElement(MORPH_RECT, cv::Size(3,3), cv::Point(1,1));
...
Chessboard::checkChessboard(const cv::Mat& image, cv::Size patternSize) const
{
    ...
    {
        cv::threshold(white, thresh, threshLevel + blackWhiteGap, 255, THRESH_BINARY);

        std::vector< std::vector<cv::Point> > contours;
        std::vector<cv::Vec4i> hierarchy;

        // Initialize contour retrieving routine
        cv::findContours(thresh, contours, hierarchy, RETR_CCOMP, CHAIN_APPROX_SIMPLE);

        std::vector<std::pair<float, int> > quads;
        getQuadrangleHypotheses(contours, quads, 1);

        cv::threshold(black, thresh, threshLevel, 255, THRESH_BINARY_INV);
